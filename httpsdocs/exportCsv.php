...
if ($dir == 'import') {
    ...
    if (isset($_FILES['file'])) {
        ...
        if ($_FILES['file']['type'] != 'application/vnd.ms-excel' ||
            !preg_match('#csv$#', $_FILES['file']['name'])) {
            report(array('success'=>false, 'type'=>'wrongtype'));
        }
...
function check($item, $option)
{
    ...
    if ($option == 'int') {
        return preg_match('#[0-9]{1,}#', $item);
    }
    if ($option == 'text') {
        return preg_match('#[a-zA-Z]{1,}#', $item);
    }
}
function update($array)
{
    ...
    $res = DB\dbQuery(
        'update `tags` set '.implode(',', $langs).', `pid`="'.$array['pid'].'", `order` = '.$count.'
        where id='.$array['id']
...
{
    $res = DB\dbQuery('DELETE from tags WHERE id='.$array['id']) or die( DB\dbQueryError() );
}
...
function insertWithId($array)
{
    ...
    $query = 'INSERT INTO tags (`order`, id, pid, type,'.implode(',', $keys).
        ') VALUES ('.$count.','.$array['id'].','.$array['pid'].', '.$array['type'].', '.implode(',', $langs).')';
    $res = DB\dbQuery($query) or die( DB\dbQueryError() );
...
function getDir()
{
    ...
    if (empty($_GET['dir'])) {
        ...
        $dir = array_search($_GET['dir'], array('import'=>'import', 'export'=>'export'));
    }
    if (!$dir) {
        die();
    }
