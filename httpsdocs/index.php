...
<?php
	...
	if(!empty($customConfig[CORENAME.'_css'])) echo '<link rel="stylesheet" type="text/css" href="'.Minify_getUri(CORENAME.'_css').'" />';
...
<script type="text/javascript" src="<?php echo EXT_PATH ?>/ext-all<?php echo is_debug_host() ? '-debug' : ''; ?>.js"></script>
<?php if(!empty($_SESSION['user']['language']) && ($_SESSION['user']['language'] != 'en')){
	...
	}else echo '<script type="text/javascript" src="'.Minify_getUri('lang-en').'"></script>';
?>
...
<?php
	...
	if(!empty($customConfig[CORENAME.'_js'])) echo '<script type="text/javascript" src="'.Minify_getUri(CORENAME.'_js').(is_debug_host() ? '&debug=1': '').'"></script>';
?>
