<?php
	...
	if(empty($_GET['z']) || ($_GET['z'] != 1)){
		...
		if($r = $res->fetch_assoc()){
			...
			readfile(CB_FILES_PATH.$r['path'].DIRECTORY_SEPARATOR.$r['content_id']);
		...
		if(!empty($ids)){
			...
			foreach($files as $f) $zip->addFile(CB_FILES_PATH.$f['path'].DIRECTORY_SEPARATOR.$f['content_id'], $f['name']);
			$zip->close();
