...
// Checking if filename corresponds to a strict format


$id = 0;
$ext = '';

// a number "nnnnn": requesting latest version of an object
if (preg_match('/^(\d+)$/', $f, $matches)) {
    $id = $matches[1];
}
// is it a version history of an object "nnnn_kkkk"
elseif (preg_match('/^(\d+)_(\d+)$/', $f, $matches)) {
    $id = $matches[1];
    $version_id = $matches[2];
}

// is it a filename with an extension (an additional file created by libreoffice during preview generation) ?
// "nnnn_abcd.ext"
elseif (preg_match('/^(\d+)\_([a-z0-9-_]+)?\.([a-z0-9]+)$/', $f, $matches)) {
    $id = $matches[1];
    $ext = $matches[3];
}

// returning a not found
else
{
