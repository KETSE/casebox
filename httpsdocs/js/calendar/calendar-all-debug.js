...
Ext.calendar.MonthView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} {values.spanCls} ext-cal-evt ext-cal-evr {cls}">',
            body,
            '</div>'
            )
            : new Ext.XTemplate(
            '<tpl if="_renderAsAllDay">',
            '<div id="{_elId}" class="{_selectorCls} {values.spanCls} {_colorCls} ext-cal-evt ext-cal-evo {cls}">',
            '<div class="ext-cal-evm">',
            '<div class="ext-cal-evi">',
            '</tpl>',
            '<tpl if="!_renderAsAllDay">',
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr {cls}">',
            '</tpl>',
            body,
            '<tpl if="_renderAsAllDay">',
            '</div>',
...
Ext.calendar.DayBodyView = Ext.extend(Ext.calendar.CalendarView, {
    ...
    getEventTemplate: function() {
        if (!this.eventTpl) {
            ...
            new Ext.XTemplate(
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} ext-cal-evt ext-cal-evr {cls}" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
            '<div class="ext-evt-bd">', this.getEventBodyMarkup(), '</div>',
            '<div class="ext-evt-rsz"><div class="ext-evt-rsz-h">&nbsp;</div></div>',
            '</div>'
            )
            : new Ext.XTemplate(
            '<div id="{_elId}" class="ext-cal-evt {_selectorCls} {_colorCls}-x {cls}" style="left: {_left}%; width: {_width}%; top: {_top}px;">',
    ...
    getEventAllDayTemplate: function() {
        if (!this.eventAllDayTpl) {
            ...
            tpl = !(Ext.isIE || Ext.isOpera) ?
            new Ext.XTemplate(
            '<div id="{_elId}" class="{_selectorCls} {_colorCls} {values.spanCls} ext-cal-evt ext-cal-evr {cls}" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
            body,
            '</div>'
            )
            : new Ext.XTemplate(
            '<div id="{_elId}" class="ext-cal-evt {cls}" style="left: {_left}%; width: {_width}%; top: {_top}px; height: {_height}px;">',
...
Ext.calendar.CalendarPanel = Ext.extend(Ext.Panel, {
    ...
    setActiveView: function(id) {
        ...
        if (id == this.id + '-edit') {
            ...
            tb = this.getTopToolbar(); if(tb) tb.show();
            this.updateNavState();
        }
