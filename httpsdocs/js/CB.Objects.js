Ext.namespace('CB');
 ...
 Ext.define('CB.Objects', {
    extend: 'CB.GenericForm'
    ,alias: 'CBObjects'
    ...
    ,initComponent: function(config){
        ...
        this.actions = {
            ...
            ,createTask: new Ext.Action({
                ...
                //,disabled: true
                ,scope: this
                ,handler: Ext.Function.createInterceptor(this.onCreateTaskClick, this.autoSaveObjectInterceptor, this)
            })
            ,upload: new Ext.Action({
                ...
                //,disabled: true
                ,scope: this
                ,handler: Ext.Function.createInterceptor(this.onUploadClick, this.autoSaveObjectInterceptor, this)
            })
            ,paste: new Ext.Action({
                ...
                ,disabled: true
                ,scope: this
                ,handler: Ext.Function.createInterceptor(this.onPasteClick, this.autoSaveObjectInterceptor, this)
            })
    ...
    ,prepareInterface: function(){
        ...
        this.grid = Ext.create(
            Ext.valueFrom(this.templateData.cfg.gridJsClass, 'CBVerticalEditGrid')
            ,{
                title: L.Details
        ...
        this.objectPanel = new CB.ObjectCardView({
            ...
            ,listeners: {
                ...
                ,loaded: function(objectPanel, activeViewItem) {
                    var tb = objectPanel.dockedItems.getAt(0);
                    ...
                    if(tb) {
                        if(Ext.isEmpty(objectPanel.loadedData) ||
                            ...
                            objectPanel.dockedItems.getAt(0).hide();
                        } else {
                            objectPanel.dockedItems.getAt(0).show();
                        }
                    }
        ...
        this.mainToolBar = this.items.first().dockedItems.getAt(0);
    ...
    ,getCurrentFieldValue: function(field_id, duplication_id){
        ed = this.topFieldSet.child('[name="f'+ field_id+'_0"]');
        if(!Ext.isEmpty(ed)) {
            return ed.getValue();
        }
    ...
    ,setFormValues: function(){
        ...
        */
        ...
        this.templateStore.each(function(r){
            if((r.get('cfg').showIn == 'top') && Ext.isDefined(this.topFieldSet)){
                ...
                if ((r.get('type') == 'date') && Ext.isString(v.value) && !Ext.isEmpty(v.value)) {
                    v.value = Ext.Date.parse(v.value.substr(0,10), 'Y-m-d');
                }

                if ((r.get('type') == 'datetime') && Ext.isString(v.value) && !Ext.isEmpty(v.value)) {
                    v.value = Ext.Date.parse(v.value, (v.value.indexOf('T') >= 0) ? 'Y-m-dTH:i:s' : 'Y-m-d H:i:s' );
                }
                ...
                if( Ext.isDefined(r.get('cfg').dependency) && !Ext.isEmpty(r.get('pid'))){
                    ...
                    pidValue = this.data.data
                        ? Ext.valueFrom(this.data.data[pidRow.get('name')], {})
                        : null;
                    if(pidValue['value']) {
        ...
        p = this.tabPanel.items.getAt(lastActiveTabIndex);

        if(!p || !p.isVisible()) {
            p = this.tabPanel.items.getAt(lastActiveTabIndex+1);
            if(!p || !p.isVisible()) {
                p = this.tabPanel.items.getAt(lastActiveTabIndex-1);
        ...
        if(Ext.isDefined(this.topFieldSet)){
            this.templateStore.each(function(r){
                if((r.get('cfg').showIn == 'top') && Ext.isDefined(r.get('cfg').dependency) && (r.get('pid') == pid) ){
                    c = this.topFieldSet.child('[name="f'+r.get('id')+'_0"]');
                    if(!Ext.isEmpty(c)){
