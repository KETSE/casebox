...
if( !in_array($script, array('login.php', 'router.php', 'preview.php', 'recover_password.php', 'download.php') ) ){
	...
	if( ($_SERVER['SCRIPT_NAME'] != '/auth.php') && !User::is_loged()){
		header('Location: /login.php');
		exit(0);
	}
}
...
if( !empty($_COOKIE['L']) && (strlen($_COOKIE['L']) == 2) ) $user_language = strtolower($_COOKIE['L']);
if( !empty($_GET['l']) && (strlen($_GET['l']) == 2) ) $user_language = strtolower($_GET['l']);
...
if( isset($_SESSION['user']['language']) && isset($GLOBALS['language_settings'][$_SESSION['user']['language']]) ) $user_language = $_SESSION['user']['language'];
elseif( !isset($GLOBALS['language_settings'][@$_SESSION['user']['language']])) $user_language = 'en';
