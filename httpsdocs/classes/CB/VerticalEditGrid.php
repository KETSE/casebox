...
class VerticalEditGrid
{
    ...
    public static function getData($objectName, &$data)
    {
        ...
        $res = DB\dbQuery(
            'SELECT
                concat(\'f\', field_id, \'_\', duplicate_id) field
                ,id
                ,`value`
                ,info
            FROM '.$objectName.'_data
            WHERE '.$id_field.' = $1',
            $data['id']
        ...
        while ($r = $res->fetch_assoc()) {
            $data['gridData']['duplicateFields'][$r['field_id']][$r['id']] = $r['pid'];
        }
        $res->close();
    ...
    public static function saveData($objectName, &$data)
    {
        ...
        if (isset($data['gridData']['duplicateFields'])) {
            $sql = 'INSERT INTO  '.$objectName.'_duplicates
                    (pid, '.$id_field.', field_id)
                VALUES ($1, $2, $3)';
        ...
        $sql = 'INSERT INTO  '.$objectName.'_data
                    ('.$id_field.', field_id, duplicate_id, `value`, info)
                VALUES ($1, $2, $3, $4, $5)
                ON DUPLICATE KEY
