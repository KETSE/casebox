...
class Favorites{
	...
	public function create($p){
		...
		DB\mysqli_query_params('insert into favorites (user_id, object_id) values($1, $2) on duplicate key update object_id = $2', array($_SESSION['user']['id'], $p->data->id)) or die(DB\mysqli_query_error());
		...
		$res = DB\mysqli_query_params($sql, array($_SESSION['user']['id'], $p->data->id) ) or die(DB\mysqli_query_error());
	...
	public function destroy($p){
		$rez = array('succes' => true, 'data' => array());
		DB\mysqli_query_params('delete from favorites where user_id = $1 and object_id = $2', array($_SESSION['user']['id'], intval($p->data) ) ) or die(DB\mysqli_query_error());
		return $rez;
	}
}
?>