...
class Log{
	...
	public function getLastLog(){
		$data = array();
		$res = DB\mysqli_query_params('select l'.USER_LANGUAGE_INDEX.' html, date from actions_log where pid is null order by `date` desc, id desc limit 50') or die(DB\mysqli_query_error());
		while($r = $res->fetch_row()) $data[] = array($r[0].' '.Util\formatPastTime($r[1]));
	...
	public static function add($p){
		...
		// get task data
		if(!empty($p['task_id'])){
			$res = DB\mysqli_query_params('select title from tasks where id = $1', $p['task_id']) or die(DB\mysqli_query_error());
		...
		$file = array();
		if(!empty($p['file_id'])){
			$res = DB\mysqli_query_params('select id, name from files where id = $1', $p['file_id']) or die(DB\mysqli_query_error());
		...
		if(!empty($to_user_ids)){
			...
			$res = DB\mysqli_query_params('select name, '.config\language_fields.', sex from users_groups where id in ('.$p['to_user_ids'].')') or die(DB\mysqli_query_error());
		...
		if(!empty($p['office_id'])){
			$res = DB\mysqli_query_params('select name from tags where id = $1', $p['office_id']) or die(DB\mysqli_query_error());
		...
		for ($lk=0; $lk < sizeof($GLOBALS['languages']); $lk++) { 
			...
			switch($p['action_type']){ //- log actions
				...
				case 13: // add case file
					...
					if(empty($p['object_id'])) $p[$l] .= L\get('toCase', $l).$case['a']; else $p[$l] .= L\get('ofCase', $l).$obj['a'].' '.L\get('ofCase', $l).$case['a'];
					break;
				...
				case 21: // add task
					...
					if(!empty($to_user_ids) && ($p['to_user_ids'] != $p['user_id'])) $p[$l] .= ' '.( (sizeof($to_user_ids) > 1) ? L\get('forUsers', $l) : L\get('forUser', $l) ).' '.$to_user_names;
					break;
				case 22: // update task
				...
				case 23: // complete task by a user
					$p[$l] .= Log::getGenderString($sex, 'Completed', $l).' '.L\get('theTask', $l).$task['a'].' '.( $obj['id'] ? L\get('for', $l).' '.$obj['type'].$obj['a'] : '').( empty($p['case_id']) ? '' : ' '.L\get('ofCase', $l).$case['a']).
		...
		DB\mysqli_query_params($sql, $values) or die(DB\mysqli_query_error());
	...
	/**/	
		$to_user_ids = array();
		if(!empty($p['remind_users'])) $to_user_ids = Util\toNumericArray($p['remind_users']); 
		if(empty($to_user_ids)) return ;
		
		$users_data = array();
		$res = DB\mysqli_query_params('select id, language_id from users_groups where id in ('.implode(',',$to_user_ids).')') or die(DB\mysqli_query_error());
		...
		foreach($users_data as $u){
			...
			switch($p['action_type']){
				...
				case 31: // aboutTaskReopened
					 switch($p['action_type']){
					 	case 21: $subject = L\get('aboutTaskCreated', $l); break; //CHECKED
					 	case 22: $subject = L\get('aboutTaskUpdated', $l); break; //CHECKED
					 	case 23: $subject = L\get('aboutTaskComplete', $l); break; //CHECKED
					 	case 24: $subject = L\get('aboutTaskDelete', $l); break; // TO BE REWIEWED
					 	case 26: $subject = L\get('aboutUserTaskStatusChange', $l); break; // depricated
					 	case 27: $subject = L\get('aboutTaskComplete', $l); break;//aboutTaskClose //CHECKED
					 	case 28: $subject = L\get('aboutTaskOverdue', $l); break;
					 	case 29: $subject = L\get('aboutTaskCompletionDecline', $l); break; //CHECKED
					 	case 30: $subject = L\get('aboutTaskCompletionOnBehalt', $l); break; //CHECKED
					 	case 31: $subject = L\get('aboutTaskReopened', $l); break; //CHECKED
					 }
					...
					$res = DB\mysqli_query_params($sql, $p['task_id']) or die(DB\mysqli_query_error());
					if($r = $res->fetch_assoc())
			...
			DB\mysqli_query_params('INSERT INTO notifications (action_type, case_id, object_id, task_id, subtype, subject, message, time, user_id) VALUES ($1, $2, $3, $4, 0, $5, $6, CURRENT_TIMESTAMP, $7)', 
							array($p['action_type'], $p['case_id'], $p['object_id'], $p['task_id'], $subject, $message, $u['id'])) or die(DB\mysqli_query_error());
		}
	}
}
?>