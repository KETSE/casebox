...
<?php
	...
	foreach ( $mailbox as $k => $mail ){
		...
		$user_id = false;
		$res = DB\mysqli_query_params('select id from users_groups where (`email` like $1) or (`email` like $2) or (`email` like $3)', array($email, '%,'.$email, $email.',%') ) or die(DB\mysqli_query_error());
		...
		DB\mysqli_query_params('insert into objects_data (object_id, field_id, duplicate_id, `value`) select $1, id, 0, $2 from templates_structure where template_id = $3 and name = $4', array($object_id, $subject, $email_template_id, '_title') ) or die(DB\mysqli_query_error());
		DB\mysqli_query_params('insert into objects_data (object_id, field_id, duplicate_id, `value`) select $1, id, 0, $2 from templates_structure where template_id = $3 and name = $4', array($object_id, $time, $email_template_id, '_date_start') ) or die(DB\mysqli_query_error());
		DB\mysqli_query_params('insert into objects_data (object_id, field_id, duplicate_id, `value`) select $1, id, 0, $2 from templates_structure where template_id = $3 and name = $4', array($object_id, $content, $email_template_id, '_content') ) or die(DB\mysqli_query_error());
		DB\mysqli_query_params('insert into objects_data (object_id, field_id, duplicate_id, `value`) select $1, id, 0, $2 from templates_structure where template_id = $3 and name = $4', array($object_id, $mail->from, $email_template_id, 'from') ) or die(DB\mysqli_query_error());
			...
			foreach($attachments as $a){
				if(!$a['attachment']) continue;
					...
					DB\mysqli_query_params('insert into files (id, content_id, `date`, `name`, `title`, cid, uid, cdate, udate) values ($1, $2, $3, $4, $5, $6, $6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)'
						,array($file_id, $f['content_id'], $time, $a['filename'], '', $user_id)) or die(DB\mysqli_query_error());
			}
		}
